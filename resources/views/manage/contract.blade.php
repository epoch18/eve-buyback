...
<div class="col-md-offset-1 col-md-10 table-responsive">
	<table id="buying" class="table table-condensed table-striped table-hover">
		...
		<thead>
			<tr>
				<th>{!! ucfirst(trans('buyback.date_issued'     )) !!}</th>
				<th>{!! ucfirst(trans('buyback.location'        )) !!}</th>
				<th>{!! ucfirst(trans('buyback.issuer'          )) !!}</th>
				<th>{!! ucfirst(trans('buyback.volume'          )) !!}</th>
				<th>{!! ucfirst(trans('buyback.asking_price'    )) !!}</th>
				<th>{!! ucfirst(trans('buyback.calculated_price')) !!}</th>
				<th>{!! ucfirst(trans('buyback.jita_price'      )) !!}</th>
				<th>{!! ucfirst(trans('buyback.contract_title'  )) !!}</th>
				<th>{!! ucfirst(trans('buyback.profit_margin'   )) !!}</th>
				<th></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			@foreach ($buying as $contract)
				...
				@endif
					...
					<td>{!! number_abbreviate($contract->totalModded                  ) !!} </td>
								...
								<table id="" class="table table-condensed table-striped table-hover">
									...
									<thead>
										<tr>
											...
											<th>{!! ucfirst(trans('buyback.total_price'       )) !!}</th>
											<th>{!! ucfirst(trans('buyback.modified_price'    )) !!}</th>
										</tr>
									</thead>
									<tbody>
